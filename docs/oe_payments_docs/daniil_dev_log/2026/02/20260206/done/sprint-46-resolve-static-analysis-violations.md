...
**Prerequisites:** Sprint 43 completed (interfaces created), Sprint 45 completed (partial fixes)
...
| # | Question | Decision |
|---|----------|----------|
| Q1 | Scope | **All 4 phases** |
| Q2 | StripeAdapter approach | **Extract helpers** (keep wide interface, delegate to helper classes) |
| Q3 | OrderRefund depth | **Full extraction** (StripeRefundApiService + RefundValidationService) |
| Q4 | ModuleConfigurationService | **Sub-interfaces** (split into focused config interfaces, update injection points) |

## Already Fixed (Sprint 45)

- IdempotentStripeAdapter: 17 PHPStan `cast.string` errors → typed array shapes
- StripeCheckoutSessionHandler: CC 10→5, NPath 256→16 → extracted helpers
- Stripe SDK `argument.type` errors → phpstan.neon suppression
- **Remaining:** 8 PHPMD class-level violations, ~21 PHPStan errors
