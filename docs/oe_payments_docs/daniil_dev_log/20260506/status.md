...
| 7 | Architecture violation flagged: opalreturns tests import concrete Stripe / PayPal classes and hard-code the `'stripe'` / `'paypal'` provider names, even though opalreturns is provider-agnostic and `src/` is clean. The pre-commit arch-guard checks only `src/`, so the leak slipped in across Sprint H, Sprint 94 and task #6. | `opalreturns` (tests + arch-guard) | ✅ Landed — Sprint 95 executed end-to-end. PHPUnit arch-guard `NoConcretePspReferencesTest` lives in `tests/Unit/Architecture/`; bash guard widened to cover `src` + `services.yaml` + `tests` (regex source shared via `tests/Architecture/psp-blacklist.regex`, DRY). New broker-spy fixture `OpalReturnsTestChain` + `BrokerSpy` + `ChainOptions` replaces the old PSP-leaky chain. Sprint H test moved to `tests/CrossModule/` with `#[Group('cross-module')]` (G4 deferred until Stripe-side translator test exists). +9 net tests; full pre-commit green. | 2026-05-06 |
