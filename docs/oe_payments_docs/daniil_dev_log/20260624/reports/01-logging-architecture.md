...
| Frontend `console.*` controllable | ✅ | esbuild `drop: ['console']` with `console.error` preserved via `pure`-list exclusion. Runtime `debug()` wrapper in `stripe_order_controller.js` and `order_submit_controller.js` — no-ops unless `data-*-stripe-debug-value="true"`. `ViewConfig::isStripeDebug()` passes the flag from PHP. Prod bundle: 0 stray `console.log/warn/debug`; 7 `console.error` calls retained intentionally. |
