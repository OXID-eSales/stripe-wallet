...
| # | Question | Decision |
|---|----------|----------|
| Q1 | Architectural approach | **A) Template Method Pattern** - Create AbstractWebhookProcessor, Stripe extends it |
| Q2 | Event routing | **A) Provider decides** - Base class calls abstract processEvent(), provider uses match/switch |
| Q3 | Migration strategy | **A) Replace completely** - Delete WebhookProcessingService, create StripeWebhookProcessor |
| Q4 | Idempotency/logging | **A) Built into abstract class** - AbstractWebhookProcessor handles internally |

---

## Confirmed Implementation Approach

1. **Create `AbstractWebhookProcessor`** in payment-component with Template Method pattern
2. **Shared logic in base class:**
   - Idempotency check (`isAlreadyProcessed()`)
   - Webhook logging (`logWebhookReceived()`, `logWebhookResult()`)
   - Overall flow orchestration
3. **Provider implements:**
   - `parseRequest()` - Parse provider-specific webhook payload
   - `validateSignature()` - Verify webhook authenticity
   - `processEvent()` - Route to specific handlers using match/switch
4. **Create `StripeWebhookProcessor extends AbstractWebhookProcessor`**
5. **Delete `WebhookProcessingService`** - move logic to StripeWebhookProcessor
6. **Update `StripeWebhookController`** to use new processor
