...
class WebhookContractFulfillmentHandlerTest extends TestCase
{
    ...
    // =========================================================================
    // STRP-AUTOCAP-REFUND: handlePaymentSucceeded must propagate captured amount
    // The opalreturns refund broker uses OXCAPTUREDAMOUNT as source of truth.
    // For auto-captured orders, Stripe only sends payment_intent.succeeded —
    // this is our one chance to persist the amount.
    // =========================================================================
     ...
     */
    public function handlePaymentSucceededRecordsCapturedAmountOnContract(): void
    {
        $providerOrderId = 'pi_auto_capture_001';
        ...
        $contract = new PaymentContract(1, 'user-auto-1', $snapshot);
        $contract->addCondition(new ContractCondition(ContractCondition::TYPE_PAYMENT_AUTHORIZED));
        $contract->transitionToNotFinished('order-auto-1');
        $contract->transitionToPending();
        $contract->setProvider('stripe', $providerOrderId);
        $contract->fulfillCondition(ContractCondition::TYPE_PAYMENT_AUTHORIZED, ['authId' => 'auth_auto']);
        $contract->commitToOrder('order-auto-1');
        ...
        $result = $handler->handlePaymentSucceeded($providerOrderId, $capturedAmount);

        $this->assertTrue($result);
        $this->assertSame($capturedAmount, $contract->getCapturedAmount(),
            'OXCAPTUREDAMOUNT must be set by handlePaymentSucceeded for auto-capture orders');
        $this->assertNotNull($contract->getCapturedAt(),
            'OXCAPTUREDAT must be set whenever capturedAmount is recorded');
    }
     ...
     * @group strp-autocap-refund
     */
    public function handlePaymentSucceededWithZeroAmountDoesNotWriteContract(): void
    {
        // Defensive: zero amount means "unknown" — do not write, do not save.
        $providerOrderId = 'pi_zero_amount';

        $this->contractFulfillmentService
            ->method('fulfillByProviderOrderId')
            ->willReturn(true);

        // Repository must NOT be hit for save when no amount is recorded.
        $this->contractRepository
            ->expects($this->never())
            ->method('save');
        ...
        $result = $handler->handlePaymentSucceeded($providerOrderId, 0.0);
     ...
     * @group strp-autocap-refund
     */
    public function handlePaymentSucceededDefaultsToZeroAmountForBackwardsCompat(): void
    {
        // Old call sites pass no amount — must not break, must not write.
        $providerOrderId = 'pi_legacy_call';
